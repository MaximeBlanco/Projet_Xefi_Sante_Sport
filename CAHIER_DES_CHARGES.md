...
- Écran "Mon profil" : historique, total de points, séries (streaks),
  répartition par sport (graphique simple).
...
│   │   ├── main.dart
│   │   ├── app.dart
│   │   ├── core/         # thème, constantes, client HTTP, config env
│   │   ├── models/       # classes de données (User, Team, Sport, Session)
│   │   ├── data/         # repositories (appels API)
...
└── server/                # API Node.js
    ├── src/
    │   ├── routes/        # un fichier par ressource (users, teams, sessions, rankings)
...
- **Base de données : SQLite via Prisma ORM** en V1 (zéro configuration,
  fichier unique, facile pour deux devs qui travaillent chacun en local).
  Le schéma Prisma sert de source de vérité pour le modèle de données —
  migrable vers PostgreSQL plus tard sans réécrire la logique métier.
- **Auth : JWT** (token renvoyé au login, stocké côté app avec
  `flutter_secure_storage`, envoyé en `Authorization: Bearer <token>`).
...
| POST    | `/sessions`                   | oui  | Enregistre une séance `{sportId, date, durationMin}` |
...
- Avant toute PR : `flutter analyze` + `flutter test` côté app,
  `npm test` (si tests présents) côté serveur doivent passer sans erreur.
...
- Un fichier de routes par ressource (`routes/sessions.js`, etc.).
- Validation des entrées à chaque route (400 si champs manquants/invalides).
- Pas de logique métier dans les routes elles-mêmes si elle dépasse
  quelques lignes — extraire dans un module dédié (ex: calcul de points).
